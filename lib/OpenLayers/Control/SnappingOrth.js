...
OpenLayers.Control.Snapping = OpenLayers.Class(OpenLayers.Control, {
    ...
    considerSnapping: function(point, loc) {
        ...
        var tolerance = this.getGeoTolerance(10, resolution);
        // now try and snap to x - y on itself
        var siblings = point.parent.components;
        var snapX = null;
        var snapY = null;
	var dx, dy,sib;
        // if already snapped, only snap if it's better and in one dimension
        var cur = best.x ? best : loc;
	for(var i=0, len=siblings.length; i<len; ++i){
           sib = siblings[i];
           if (point === sib){ continue; }
           dx = Math.abs(sib.x - loc.x);
           dy = Math.abs(sib.y - loc.y);
           if (dx <= tolerance){
             snapX = sib.x;
           }
           if (dy <= tolerance){
